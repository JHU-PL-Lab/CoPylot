...
let rec get_uid_hashtbl (m : Abstract.modl) =
  ...
  match m with
  | Module(body, _) ->
    let tbl = collect_uids_stmt_lst (Counter_hashtbl.create 10) body in
    tbl

and collect_uids_stmt (tbl : Abstract.annotated_stmt Counter_hashtbl.t) {uid=u;exception_target=except;multi=in_loop;body} =
  let open Abstract in
  match body with
  | Assign (_, {body = Literal(FunctionVal(_, func_body)); _}) ->
    let new_tbl = collect_uids_stmt_lst tbl func_body in
    Counter_hashtbl.add new_tbl u {uid=u;exception_target=except;multi=in_loop;body=body}; new_tbl
  | Assign (_, _)
  | Return (_)
  | Print (_, _, _)
  | Raise (_)
  ...
  | GotoIfNot (_)
    ->
    Counter_hashtbl.add tbl u {uid=u;exception_target=except;multi=in_loop;body}; tbl
...
let rec norm_get_uid_hashtbl (m : Normalized.modl) =
  ...
  match m with
  | Module(body, _) ->
    let tbl = norm_collect_uids_stmt_lst (Counter_hashtbl.create 10) body in
    tbl

and norm_collect_uids_stmt (tbl : Normalized.annotated_stmt Counter_hashtbl.t) {uid=u;exception_target=except;multi=in_loop;body} =
  let open Normalized in
  match body with
  | Assign (_, {body = Literal(FunctionVal(_, func_body)); _}) ->
