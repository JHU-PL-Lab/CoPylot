%{
    ...
    let annot = Python2_ast.Pos.of_pos
...
%start file_input
%type <Python2_ast.Pos.t Python2_ast.modl> file_input

%start stmt_input
%type <Python2_ast.Pos.t Python2_ast.stmt list> stmt_input
...
varargslist:
  ...
  (* Remove default arguments and tuplization
  | fpdef EQ test
      {
...
(* "Function parameter definition", probably*)
fpdef:
...
(* Allows variable numbers of arguments
fpvarargs:
...
small_stmt:
  | expr_stmt   { $1 } (* List of tests and assignments *)
  | print_stmt  { $1 }
  (*
  | del_stmt    { $1 }*)
  | pass_stmt   { $1 }
...
print_stmt:
  ...
  (* Make printing just go to stdout
...
(* Only used by list comprehensions
testlist_safe:
