  ...
  let rest =
    ...
    | Return (value)
      -> begin
          match value with
          | None -> []
          | Some(e) -> collect_uids_sexpr e
        end
    | Print (dest, values, _)
      ->
      let big_list =
        List.concat (List.map collect_uids_sexpr values) in
      begin
        match dest with
        | None -> big_list
...
and collect_uids_cexpr c =
  let rest =
    match c.body with
    | Call (func, args)
      -> (collect_uids_sexpr func) @
         (List.concat (List.map collect_uids_sexpr args))
    | Attribute (obj, _)
      -> collect_uids_sexpr obj
    | List (elts)
    | Tuple (elts)
      -> List.concat (List.map collect_uids_sexpr elts)
    | SimpleExpr (value)
      -> collect_uids_sexpr value
  in
  c.uid::rest

and collect_uids_sexpr s =
  [s.uid]
;;
